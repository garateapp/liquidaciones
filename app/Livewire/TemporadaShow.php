...
public array $costRows = []; // cada fila: ['name' => ..., 'total' => float, 'unit' => float]
...
public function loadResumen(): void
{
    // Filtros base
    $base = DB::table('balancemasas')
        ->where('temporada_id', $this->temporada->id)
        ->whereIn('exportadora', ['Greenex SpA', '22']);

    // Aplica tu scope de filtros (si es query scope, clónalo)
    // $base = $base->filter1($this->filters);  // si es un scope local de Eloquent, hazlo sobre el modelo:
    // Como usamos Query Builder, replicamos el scope manualmente o hacemos la suma con Eloquent:
    // Para mantenerlo simple, usa Eloquent con selectRaw:
    $sum = \App\Models\Balancemasa::query()
        ->where('temporada_id', $this->temporada->id)
        ->whereIn('exportadora', ['Greenex SpA', '22'])
        ->filter1($this->filters)
        ->selectRaw('
            SUM(peso_neto)                           AS total_peso,
            SUM(precio_unitario * peso_neto)         AS ingresos_total
        ')
        ->first();

    $totalPeso = (float) ($sum->total_peso ?? 0);
    $ingresos  = (float) ($sum->ingresos_total ?? 0);
    $valorUnit = $totalPeso > 0 ? $ingresos / $totalPeso : 0;
    ...
    // ===== COSTOS =====
    // Construimos filas por método. Ajusta nombres de tablas/cols a tu esquema real.
    ...
    foreach ($this->costos as $costo) {
        ...
        switch ($costo->metodo) {
            case 'TPK': // Tarifa por KG
                // costotarifakilos: [costo_id, tarifa_kg]
                $tarifa = (float) optional(
                    DB::table('costotarifakilos')->where('costo_id', $costo->id)->first()
                )->tarifa_kg;

                if ($tarifa > 0 && $totalPeso > 0) {
                    $total = $totalPeso * $tarifa;
                }
                break;

            case 'TPC': // Tarifa por Caja
                // Necesitamos SUM(cantidad) * tarifa_caja
                $tarifaCaja = (float) optional(
                    DB::table('costotarifacajas')->where('costo_id', $costo->id)->first()
                )->tarifa_caja;

                if ($tarifaCaja > 0) {
                    $sumCajas = \App\Models\Balancemasa::query()
                        ->where('temporada_id', $this->temporada->id)
                        ->whereIn('exportadora', ['Greenex SpA', '22'])
                        ->filter1($this->filters)
                        ->sum('cantidad');
                    $total = ((float) $sumCajas) * $tarifaCaja;
                }
                break;

            case 'TPE': // Costo por código de embalaje
                // costo_embalaje_codes: [temporada_id, costo_id, c_embalaje, costo_por_caja]
                // total = sum( cantidad * costo_por_caja ) join por c_embalaje
                $total = DB::table('balancemasas AS b')
                    ->join('costo_embalaje_codes AS e', function ($j) use ($costo) {
                        $j->on('e.c_embalaje', '=', 'b.c_embalaje')
                          ->where('e.costo_id', '=', $costo->id)
                          ->whereColumn('e.temporada_id', 'b.temporada_id');
                    })
                    ->where('b.temporada_id', $this->temporada->id)
                    ->whereIn('b.exportadora', ['Greenex SpA', '22'])
                    // si quieres replicar filter1 aquí, añade las condiciones manuales del scope
                    ->sum(DB::raw('b.cantidad * e.costo_por_caja'));
                break;

            case 'TPCL': // Tarifa por color (kg * tarifa_kg por color)
                // costotarifacolors: [temporada_id, costo_id?, color, tarifa_kg]
                // total = sum_por_color( kgs_color * tarifa_color )
                $porColor = DB::table('balancemasas')
                    ->select('color', DB::raw('SUM(peso_neto) AS kgs'))
                    ->where('temporada_id', $this->temporada->id)
                    ->whereIn('exportadora', ['Greenex SpA', '22'])
                    ->groupBy('color')
                    ->get()
                ...
                $tarifas = DB::table('costotarifacolors')
                    ->where('temporada_id', $this->temporada->id)
                    ->get()
                    ->keyBy('color');

                $acc = 0.0;
                foreach ($porColor as $color => $row) {
                    $t = (float) ($tarifas[$color]->tarifa_kg ?? 0);
                    if ($t > 0) $acc += ((float) $row->kgs) * $t;
                }
                $total = $acc;
                break;

            case 'TPT': // Transporte (marítimo/aéreo/terrestre) por kg (ejemplo simple)
                // exportacions: [costo_id, type (maritimo/aereo/terrestre), precio_usd]
                // Supongamos que el tipo de transporte aplicable está en balancemasas.tipo_transporte
                // total = sum( kg * tarifa_type ) según el type
                $tarifas = DB::table('exportacions')
                    ->where('costo_id', $costo->id)
                    ->pluck('precio_usd', 'type'); // ['maritimo'=>..., 'aereo'=>..., 'terrestre'=>...]

                $porTipo = DB::table('balancemasas')
                    ->select('tipo_transporte', DB::raw('SUM(peso_neto) AS kgs'))
                    ->where('temporada_id', $this->temporada->id)
                    ->whereIn('exportadora', ['Greenex SpA', '22'])
                ...
                foreach ($porTipo as $r) {
                    $t = (float) ($tarifas[strtolower($r->tipo_transporte)] ?? 0);
                    if ($t > 0) $acc += ((float) $r->kgs) * $t;
                }
                $total = $acc;
                break;

            case 'PSF': // % sobre FOB (ej: % de ingresos)
                // costoporcentajefobs: [costo_id, porcentaje]
                $pct = (float) optional(
                    DB::table('costoporcentajefobs')->where('costo_id', $costo->id)->first()
                )->porcentaje;

                if ($pct !== 0.0) {
                    $total = $ingresos * ($pct / 100.0);
                }
                break;
        ...
        $rows[] = [
            'name' => $costo->name,
            'total' => $total,
            'unit'  => $totalPeso > 0 ? ($total / $totalPeso) : 0.0,
        ];
    }
