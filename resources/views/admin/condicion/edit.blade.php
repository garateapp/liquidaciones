    ...
    <div class="bg-white shadow-lg rounded overflow-hidden">
        ...
        <div class="px-6 py-4">
                    ...
                    <div id="options-container">
                        @foreach($condicionproductor->opcions as $index => $opcion)
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <button type="button" class="delete-option text-slate-800 hover:text-blue-600 text-sm bg-white hover:bg-slate-100 border border-slate-200 rounded-r-lg font-medium px-4 py-2 inline-flex space-x-1 items-center" data-url="{{ route('admin.opcions.destroy', $opcion->id) }}">
                                    <span>
            ...
            <div class="flex justify-center mt-6">
                <button class="items-center focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 sm:mt-0 px-4 py-2 text-xl bg-blue-500 hover:bg-blue-700 text-white rounded">
        ...
        document.addEventListener('DOMContentLoaded', function() {
          ...
          document.getElementById('add-option').addEventListener('click', function() {
              ...
              newOption.innerHTML = `
                  ...
                  <button type="button" class="cancel-option text-slate-800 hover:text-blue-600 text-sm bg-white hover:bg-slate-100 border border-slate-200 rounded-r-lg font-medium px-4 py-2 inline-flex space-x-1 items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                    </svg>
                  </button>
          ...
          document.getElementById('options-container').addEventListener('click', function(event) {
              // Verificar si el clic ocurrió en el botón "Cancelar" o en su SVG
              if (event.target && event.target.closest('.cancel-option')) {
                  var optionDiv = event.target.closest('div[data-index]');
                  if (optionDiv) {
                      optionDiv.remove();
                      optionIndex--;  // Decrementar el índice cuando se elimina una opción
                  }
              }
      
              // Verificar si el clic ocurrió en el botón "Eliminar" o en su SVG
              if (event.target && event.target.closest('.delete-option')) {
                  var confirmed = confirm('¿Está seguro de que desea eliminar esta opción?');
                  if (confirmed) {
                      var button = event.target.closest('.delete-option');
                      ...
                      .then(response => {
                          if (response.ok) {
                              ...
                              // Redireccionar a la vista de edición después de eliminar
                              window.location.href = "{{ route('admin.condicionproductors.edit', $condicionproductor->id) }}";
                          } else {
